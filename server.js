...
app.post('/api/activities/:id/registrations/email', authMiddleware, requireRole('admin','tresoriere','secretaire','delegue'), (req, res) => {
  ...
  if (!act) return res.status(404).json({ error: 'Activité introuvable' });
  const rows = db.prepare(`SELECT u.id, u.prenom, u.email FROM activity_registrations ar
    JOIN users u ON u.id = ar.user_id WHERE ar.activity_id = ?`).all(req.params.id);
...
app.get('/api/admin/cartes', authMiddleware, requireRole(...CARTE_ROLES), (req, res) => {
  const members = db.prepare(`
    SELECT id, prenom, nom, email, telephone, adresse, plan, role, actif, titre_comite, date_inscription, membre_depuis, photo_url, photo_original_url,
           carte_photo_approuvee, carte_photo_deja_approuvee, carte_notif_renouv
    FROM users WHERE (phantom IS NULL OR phantom=0)
    ORDER BY nom, prenom
  `).all();
  const now = new Date();
...
function donsRecomputeNbPersonnes(foyerId) {
  const n = db.prepare('SELECT COUNT(*) AS c FROM dons_foyer_personnes WHERE foyer_id=?').get(foyerId).c;
...
function donsCorrespondances(membre) {
  ...
  if (membre.telephone && membre.telephone.trim()) {
    ...
    if (digits.length >= 7) {
      db.prepare(`SELECT id, prenom, nom, telephone FROM users WHERE id != ? AND actif=1 AND telephone IS NOT NULL AND telephone != ''`)
        .all(membre.id)
        .filter(r => r.telephone.replace(/\D/g, '') === digits)
        .forEach(r => correspondances.push({ champ: 'téléphone', valeur: membre.telephone, avec: { id: r.id, prenom: r.prenom, nom: r.nom } }));
    }
  }
...
app.post('/api/dons/foyers/:id/personnes', authMiddleware, (req, res) => {
...
app.post('/api/dons/scan', authMiddleware, (req, res) => {
  ...
  // Fiche + détection de correspondances (adresse/téléphone partagés avec un autre membre) —
  // renvoyées dans TOUTES les réponses à partir d'ici, succès ou blocage, pour que le poste de
  // scan puisse toujours afficher/corriger la fiche de la carte présentée.
  const fiche = { id: membre.id, prenom: membre.prenom, nom: membre.nom, photo_url: membre.photo_url,
    date_naissance: membre.date_naissance, adresse: membre.adresse, telephone: membre.telephone };
  const correspondances = donsCorrespondances(membre);

  if (!donCarteValide(membre)) {
    logScan('bloque_carte_invalide', null, 'Carte expirée ou non approuvée');
    return res.status(403).json({ error: `La carte de ${membre.prenom} ${membre.nom} n'est pas valide (expirée ou photo non approuvée).`, fiche, correspondances });
  }
...
app.post('/api/scan/unified', authMiddleware, (req, res) => {
  ...
  function tryCard() {
    ...
    if (!activity_id) {
      ...
      // Charger les activités du jour uniquement (±2h flexibilité pour minuit)
      // Heure de Toronto
      const nowToronto = new Date(new Date().toLocaleString('en-US', {timeZone:'America/Toronto'}));
      const todayStr = nowToronto.toISOString().slice(0, 10);
      const yesterdayStr = new Date(nowToronto.getTime() - 2*60*60*1000).toISOString().slice(0, 10);
      const tomorrowStr = new Date(nowToronto.getTime() + 2*60*60*1000).toISOString().slice(0, 10);
      const memberActivities = db.prepare(`
        SELECT a.id, a.titre, a.date_debut, a.lieu, a.prix, a.paiement_requis,
          ...
          (SELECT COUNT(*) FROM tickets WHERE activity_id=a.id AND (user_id=? OR acheteur_email=?) AND payment_status='paid' AND statut='actif') AS nb_billets
        FROM activities a
        WHERE a.statut IN ('planifiee','en_cours')
          AND substr(a.date_debut, 1, 10) BETWEEN ? AND ?
        ORDER BY a.date_debut LIMIT 20
      `).all(userId, userId, userId, member.email, yesterdayStr, tomorrowStr);
